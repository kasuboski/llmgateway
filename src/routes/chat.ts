...
// OpenAI-compatible chat completions proxy with quota enforcement
chat.post('/chat/completions', authMiddleware, async c => {
  const { AI_GATEWAY_ACCOUNT_ID, AI_GATEWAY_ID, AI_GATEWAY_TOKEN, GATEWAY_KV } = c.env;
  const user = c.get('user') as UserConfig;

  try {
    // Parse the request body to get model and estimate cost
    const requestBodyText = await c.req.text();
    const requestBody = JSON.parse(requestBodyText);
    const model = requestBody.model || 'openai/gpt-4o-mini';

    // Estimate cost for quota check
    const estimatedCost = estimateCostFromRequest(model, requestBody);
    ...
    if (!quotaCheckResult.allowed) {
      if (quotaCheckResult.reason === 'user_quota_exceeded') {
        ...
        const remainingOrgUsd =
          organizationConfig!.monthly_budget_usd - quotaCheckResult.orgQuotaRecord.month_usage_usd;
        return c.json(
          {
            error: {
